    ...
    'ping': {
        ...
        'notification': {
            'problem': {
                ...
                'level': 'warning',
                #'curl_command': "curl -v -H 'Content-type: application/json' -H 'Accept: application/json' -d 'NMS Notifications'  http://143.244.142.182:3006 -X POST --http1.1",
                'email_subject': _(
                    '[{site.name}] PROBLEM: {notification.target} {notification.verb}'
                    #'curl -X POST -H "Content-Type: application/json" -d \'{"text": "[{site.name}] PROBLEM: {notification.target} {notification.verb}"}\' http://143.244.142.182'
                ),
                'message': _(
