'use strict';var gettext=window.gettext||function(word){return word;};var interpolate=interpolate||function(){};const deviceId=getObjectIdFromUrl();const commandApiUrl=`${owControllerApiHost.origin}${owCommandApiEndpoint.replace('00000000-0000-0000-0000-000000000000', deviceId)}`;const commandWebSocket=new ReconnectingWebSocket(`${getWebSocketProtocol()}${owControllerApiHost.host}/ws/controller/device/${deviceId}/command`,null,{debug:false,automaticOpen:false,});django.jQuery(function($){if(isDeviceRecoverForm()){return;}
commandWebSocket.open();let selector=$('#id_command_set-0-type'),showFields=function(){var fields=$('#command_set-group fieldset > .form-row:not(.field-type):not(.field-params), #command_set-group .jsoneditor-wrapper'),value=selector.val();if(!value){fields.hide();}else{$('#command_set-2-group fieldset .dynamic-command_set-2:first');fields.show();}};selector.change(function(){showFields();});$('#id_command_set-0-input').one('jsonschema-schemaloaded',function(){showFields();initCommandDropdown($);initCommandOverlay($);initCommandWebSockets($);});});function initCommandDropdown($){$(function(){let widgetElement,objectTools=$('.object-tools'),owCommandBtns='',schema=django._schemas[$('#id_command_set-0-input').data('schema-url')];if(objectTools.length===0){return;}
...
$('#ow-command-confirm-text').html(confirmation_message);$('html').css('overflow-y','hidden');$('#ow-command-confirm-dialog-wrapper').removeClass('ow-hide');$('#ow-command-confirm-yes').focus();},hide:function(){$('#ow-command-confirm-dialog-wrapper').addClass('ow-hide');},add:function(){let confirmationElements=`
            ...
            <button id="ow-command-overlay-close"><img class="icon" src="/static/admin/img/icon-deletelink.svg"></button>`;$('#command_set-0 .form-row.field-input').prepend(elements);});$(function(){let buttonElement=`
...
$('#device_form').on('click','#ow-command-confirm-yes',function(e){e.preventDefault();$('#ow-command-confirm-dialog-wrapper').addClass('ow-hide');let data={"type":$('#id_command_set-0-type').val(),"input":$('#id_command_set-0-input').val()};$.ajax({type:'POST',url:commandApiUrl,headers:{'X-CSRFToken':$('input[name="csrfmiddlewaretoken"]').val()},dataType:'json',xhrFields:{withCredentials:true},data:data,crossDomain:true,beforeSend:function(){$('#loading-overlay').show();},complete:function(){if(!isRecentCommandsAbsent()){$('#loading-overlay').fadeOut(250);}},success:function(response){closeOverlay();updateRecentCommands($,response);resetCommandForm();location.assign('#command_set-2-group');},error:function(){$('#ow-command-overlay-validation-error').addClass('ow-hide');$('#ow-command-overlay-request-error').removeClass('ow-hide');}});});$('#command_set-group').on('keypress keyup keydown','.jsoneditor-wrapper input',function(e){if(e.keyCode===13){let execButton=$('#ow-command-submit-btn');execButton.focus();$(e.target).focus();if(e.type==='keyup'){execButton.trigger('click');}
e.preventDefault();return false;}});$('body').keyup(function(e){if($('#command_set-group:visible').length!==0){if(e.keyCode===27){closeOverlay();}}});function closeOverlay(){$('#id_command_set-0-type').val('reboot');$('#id_command_set-0-type').trigger('change');$('#command_set-group').css('display','none');$('.ow-command-overlay-errornote').addClass('ow-hide');commandConfirmationDialog.hide();$('html').css('overflow-y','');$('#send-command').focus();}
...
function initCommandWebSockets($){commandWebSocket.addEventListener('message',function(e){let data=JSON.parse(e.data);if(data.model!=='Command'){return;}
